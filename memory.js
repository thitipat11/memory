...
function beforeplayRoutineBegin(snapshot) {
  return async function () {
    ...
    for (const thisComponent of beforeplayComponents)
...
function loopmemoryLoopBegin(loopmemoryLoopScheduler, snapshot) {
  return async function() {
    ...
    for (const thisLoopmemory of loopmemory) {
...
function trialmemoryRoutineBegin(snapshot) {
  return async function () {
    ...
    trialmemoryComponents.push(keyspace);
    
    for (const thisComponent of trialmemoryComponents)
      if ('status' in thisComponent)
        thisComponent.status = PsychoJS.Status.NOT_STARTED;
    return Scheduler.Event.NEXT;
  }
...
function trialmemoryRoutineEachFrame() {
  return async function () {
    ...
    frameRemains = 0.0 + 3 - psychoJS.window.monitorFramePeriod * 0.75;// most of one frame period left
    if (keyspace.status === PsychoJS.Status.STARTED && t >= frameRemains) {
      keyspace.status = PsychoJS.Status.FINISHED;
        }
    ...
    if (keyspace.status === PsychoJS.Status.STARTED) {
      ...
      if (_keyspace_allKeys.length > 0) {
        ...
        keyspace.duration = _keyspace_allKeys[_keyspace_allKeys.length - 1].duration;
        // was this correct?
        if (keyspace.keys == '') {
            keyspace.corr = 1;
        } else {
...
function trialmemoryRoutineEnd(snapshot) {
  return async function () {
    ...
    // update the trial handler
    if (currentLoop instanceof MultiStairHandler) {
      currentLoop.addResponse(keyspace.corr, level);
    }
    psychoJS.experiment.addData('keyspace.keys', keyspace.keys);
    psychoJS.experiment.addData('keyspace.corr', keyspace.corr);
    if (typeof keyspace.keys !== 'undefined') {  // we had a response
        psychoJS.experiment.addData('keyspace.rt', keyspace.rt);
        psychoJS.experiment.addData('keyspace.duration', keyspace.duration);
        }
...
function anwerRoutineBegin(snapshot) {
  return async function () {
    ...
    for (const thisComponent of anwerComponents)
      if ('status' in thisComponent)
        thisComponent.status = PsychoJS.Status.NOT_STARTED;
    return Scheduler.Event.NEXT;
  }
